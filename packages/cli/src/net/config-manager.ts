...
import { isRecord, throwIfAuthError } from './api-utils.js';
...
export class ConfigManager {
  ...
  async init(): Promise<void> {
    ...
    if (this.options.connectionId) {
      ...
      this.accountConnectionId = undefined;
      this.logger.info('Stored connection not found; creating a new account connection...');
    }

    // No connectionId (or stored id was 404): create an account connection
  ...
  async setConfig(body: Record<string, unknown>): Promise<ConnectionNetworkConfig | null> {
    if (this.localOnly) {
      ...
      const rules = Array.isArray(body['rules']) ? (body['rules'] as string[]) : (current?.rules ?? []);
      this.config = {
